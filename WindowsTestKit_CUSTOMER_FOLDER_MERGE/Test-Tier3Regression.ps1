...
`$bootstrapAsts = `$scriptAst.FindAll({ param(`$n) `$n -is [System.Management.Automation.Language.IfStatementAst] -and (`$n.Extent.Text -match 'Win32NativeMergeHelper' -or `$n.Extent.Text -match 'Win32DurableJournalHelper') -and `$n.Extent.Text -match 'Add-Type' }, `$false)
foreach (`$bAst in `$bootstrapAsts) { Invoke-Expression `$bAst.Extent.Text }
